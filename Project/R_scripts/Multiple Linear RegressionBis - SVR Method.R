...
data <- cbind(Date_Range, X2_BinanceCoin_MC,
...
library(glmnet)
x_vars = model.matrix(X26_Tether_MC~., data)[,-1]
y_var = data$X26_Tether_MC
par(mfrow = c(1,2))
fit_lasso = glmnet(x_vars,y_var, alpha = 1)

lambda_seq <- 10^seq(2, -2, by = -100)
set.seed(1000)
train = sample(1:nrow(x_vars), nrow(y_var)/2)


x_test = (-train)
y_test = y_var[test]

output <- cv.glmnet(x_vars[train,], y_var[train], alpha = 1, lambda = lambda_seq)
